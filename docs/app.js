// Minimal patch version of app.js
// - Author single name in PDF header
// - Robust boot timing (window.load)
// - (Rest of logic unchanged from your working build)
...
let state = {
  caseId: 0, files: [], merkle: null, decisionHash: null,
  anchorTx: null, execTx: null, signatures: [], audit: []
};

const enc = new TextEncoder();
async function sha256Hex(buf){ const ab = await crypto.subtle.digest("SHA-256", buf);
...
function rnd(bytes=32){ const a=new Uint8Array(bytes); crypto.getRandomValues(a);
...
function short(hex, head=10, tail=8){ if(!hex||typeof hex!=="string") return String(hex);
  if(hex.length<=head+tail+3) return hex; return hex.slice(0,head)+"…"+hex.slice(-tail); }
...
async function merkleRoot(hexes){
  if(!hexes.length) return null; let layer=[...hexes];
  while(layer.length>1){ if(layer.length%2===1) layer.push(layer[layer.length-1]);
    const next=[]; for(let i=0;i<layer.length;i+=2){
      const a=layer[i].slice(2), b=layer[i+1].slice(2);
      const buf=new TextEncoder().encode(a+b); next.push(await sha256Hex(buf)); }
    layer=next; }
  return layer[0];
}
function merkleProofFor(index, hexes){
  if(hexes.length===0) return []; let layer=[...hexes], path=[], idx=index;
  while(layer.length>1){ if(layer.length%2===1) layer.push(layer[layer.length-1]);
    const isRight=idx%2===1, pairIdx=isRight?idx-1:idx+1;
    path.push({ side:isRight?"left":"right", hash:layer[pairIdx] });
    idx=Math.floor(idx/2); const next=new Array(Math.ceil(layer.length/2)).fill("0x?"); layer=next; }
  return path;
}
...
function bootstrapPresets(){ caseSelect.innerHTML=""; Object.entries(PRESETS).forEach(([k,v])=>{
  const o=document.createElement("option"); o.value=k; o.textContent=v.label; caseSelect.appendChild(o); });
  caseSelect.value="construction"; applyPreset(caseSelect.value); }
function applyPreset(key){
  const p=PRESETS[key]; if(!p) return; caseNotes.value=p.notes; caseIcon.textContent=p.icon||"⚖️";
...
function logPresetHints(stage){ const p=PRESETS[caseSelect.value]; if(!p) return;
...
btnAbout?.addEventListener("click", openAbout); backdrop?.addEventListener("click", closeAbout);
...
async function generateAwardPDF(){
  ...
  const evidenceRows = state.files.length
    ? state.files.map((f,i)=>`<tr><td>${i+1}</td><td>${f.name}</td><td style="text-align:right">${f.size.toLocaleString()} B</td><td>${short(f.hash)}</td></tr>`).join("")
  ...
  node.style.position="relative"; node.style.padding="16px"; node.style.background="#ffffff"; node.style.color="#111111";
  node.innerHTML = `
  <style>
  ...
  <div class="header">
    ...
    <div class="small"><b>Author:</b> Dokyung (DK) Kim &nbsp; | &nbsp; <b>ORCID:</b> <a href="https://orcid.org/0009-0009-4182-4974">0009-0009-4182-4974</a></div>
  </div>
  ...
  const opt = {
    margin: 10, filename: `adr-award-${state.caseId || "demo"}.pdf`,
...
btnExport.onclick = ()=>{
  ...
  const url = URL.createObjectURL(blob); const a=document.createElement("a"); a.href=url; a.download=`adr-audit-${Date.now()}.json`; a.click();
...
fileInput.onchange = async (e)=>{
  ...
  if(items.length){
    ...
    eList.querySelectorAll("button.file").forEach(btn=>{
      btn.onclick=()=>{ const idx=Number(btn.dataset.idx); const path=merkleProofFor(idx, items.map(x=>x.hash));
...
// Final boot — run after full page load to avoid race conditions
